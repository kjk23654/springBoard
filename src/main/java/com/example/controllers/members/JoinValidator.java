...
import com.example.commons.validators.MobileValidator;
import com.example.commons.validators.PasswordValidator;
...
public class JoinValidator implements Validator, MobileValidator, PasswordValidator { // 회원가입검증

    private final MemberRepository memberRepository;

    @Override
    public boolean supports(Class<?> clazz) { // 커맨드 객체 검증
        ...
        if (agrees != null && agrees.length > 0) {
            for(boolean agree : agrees) {
                if(!agree) {
                    ...
                    // reject() = 개별 프로퍼티가 아닌 객체 자체에 에러코드를 추가함(=글로벌 에러)
                    break;
                }
            }
        }
